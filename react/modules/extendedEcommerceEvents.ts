  ...
  switch (e.data.eventName) {
    case 'vtex:categoryView': {
      ...
      const categoryViewObject = {
        ...
        CategoryListedProducts: products.map(product =>
          product.productId.toString()
        ),
        CategoryLevel1: categoryNames?.[0],
        CategoryLevel2: categoryNames?.[1],
        CategoryLevel3: categoryNames?.[2],
        CategoriesPath: replaceAll(
          getCategory(products?.[0].categories) ?? '',
          '/',
          ','
        ),
      }

      const categoryEventObject = [
        'track',
        'Viewed Category',
        categoryViewObject,
      ]
